...
public class CharacteristicsImpl implements Repository<Long, Characteristics> {
    ...
    public Optional<Characteristics> findById(Long id) {
        try {
            return Optional.ofNullable(template.queryForObject(
                    ...
                    Map.of("id", id),
                    (rs, numRow) -> new Characteristics(rs.getLong("id"),
                                                        rs.getString("name"),
                                                        rs.getString("value")
                    )
            ));
        } catch (EmptyResultDataAccessException ex) {
            log.error("Characteristics with id " + id + " not found");
            throw new ResourceNotFoundException("Characteristics with id " + id + " not found");
        }
    ...
    public Characteristics findByFields(Characteristics entity) {
        try {
            return template.queryForObject(
                    "select * from characteristics c where c.name = :name and c.value = :value",
                    Map.of("name",
                           entity.getName(),
                           "value",
                           entity.getValue()
    ...
    public List<Characteristics> findAll() {
        return template.query("select * from characteristics",
                              Map.of(),
                              new BeanPropertyRowMapper<>(Characteristics.class)
        );
    }

    @Override
    public Page<Characteristics> findAll(Pageable pageable) {
        return new PageImpl<>(template.query("select * from characteristics",
                                             Map.of(),
    ...
    public Characteristics save(Characteristics entity) {
        ...
        template.update(
                "insert into characteristics(name, value) values(:name, :value)",
    ...
    public Boolean update(Characteristics entity) {
        return template.update(
                "update characteristics set name = :name, value = :value where id = :id",
                Map.of("name",
                       entity.getName(),
                       "value",
                       entity.getValue(),
    ...
    public Boolean delete(Characteristics entity) {
        return template.update("delete from characteristics where id = :id",
    ...
    @Override
    public int countAll() {
        return template.query("select count(*) from characteristics",
